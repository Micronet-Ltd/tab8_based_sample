...
JNIEXPORT jobject JNICALL Java_com_micronet_smarttabsmarthubsampleapp_SerialPort_open
    ...
    /* Opening device */
    {
        ...
        LOGD("Opening serial port %s, flags: 0x%x", path_utf, O_RDWR | O_NOCTTY | flags);
        fd = open(path_utf, O_RDWR | O_NOCTTY | flags | O_NONBLOCK);
...
JNIEXPORT void JNICALL Java_com_micronet_smarttabsmarthubsampleapp_SerialPort_config
    ...
    speed_t speed;
    struct termios cfg;
    unsigned int canonical_baud;
    LOGD("configuring baudrate to %d", baudrate);
    ...
    /* Configure device */
    {
        ...
        LOGD("Continue configuring serial port before cfmakeraw(&cfg)");
        cfmakeraw(&cfg);
        cfsetispeed(&cfg, speed);
        cfsetospeed(&cfg, speed);
//        cfg.c_cflag =  (speed | CS8 | CLOCAL | CREAD | PARENB | PARODD) & ~(CSTOPB | CRTSCTS);
        cfg.c_cflag =  (speed | CS8 | CLOCAL | CREAD) & ~(CSTOPB | CRTSCTS);
        LOGD("Configuring serial port, baudrate: %d", baudrate);
        LOGD("Configuring cfg.c_cflag: (speed | CS8 | CLOCAL | CREAD) & ~(CSTOPB | CRTSCTS);");
        cfg.c_iflag = 0;
        cfg.c_oflag = 0;
